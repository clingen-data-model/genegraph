(ns genegraph.transform.clinvar.variation-new
    ...
    (assoc event
           ...
           {:id vd-iri
            ...
            :extensions (common/fields-to-extension-maps
                         (merge (dissoc content :id :release_date :name :content)
                                {:clinvar_variation clinvar-variation-iri}))
            :description (:name content)
            :xrefs [(str (get prefix-ns-map "clinvar") (:id content))
            ...
            ;;  :value_id ()
            :value (let [vrs-ret (get-vrs-variation-map {:expression (-> event ::prioritized-expression :expr)

...
(defn eventify [input-map]
  ...
  #_{:genegraph.sink.event/value (json/generate-string input-map)
...
(defn test-event-process-jsonld! [variant-message]
  (-> variant-message
      ...
      ;; (#(do (log/info :event %) %))
      (#(event/add-to-db! %))
      #_(#((:enter ser/add-graphql-params-interceptor) %))
      #_(#((:enter ser/add-graphql-serialization-interceptor) %))
      #_(#((:leave event-recorder/record-event-interceptor) %))))
